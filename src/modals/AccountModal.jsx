...
import Account from '../modelUI/Account';

const AccountModal = ({ show, onHide, model, action }) => {
    ...
    const initialValues = {
        image: model.avata ?? "/defaultImage.jpg",
        name: model.name ?? "",
    ...
    useEffect(() => {
        if (model.avata != "") {
            if (model.avata.startsWith("/" || model.avata.startsWith("http"))) {
                setFileList({
                    status: 'done',
                    url: model.image,
                });
